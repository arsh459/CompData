...
import { getUpdatedKCal, getUpdatedMacros } from "./utils/macroUpdates";
import { updateTasksInDb } from "./utils/saveUtils";

export const reconcileNutritionMain = async () => {
  // get all nutrition tasks
  const allNutritionTasks = await getAllTasksByType("nutrition");
  console.log("all", allNutritionTasks.length);

  const updatedTasks: Task[] = [];
  let subtaskObjs: { [id: string]: SubTask | undefined } = {};
  // for each nutrition task, loop over subtasks
