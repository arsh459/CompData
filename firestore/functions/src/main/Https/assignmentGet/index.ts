...
import { AssignmentGET } from "./interface";
...
export const assignmentGetFunc = functions
  .region("asia-south1")
  .https.onRequest((request, response) => {
    corsHandler(request, response, async () => {
      // console.log(request.body);
      try {
        const body = request.query as AssignmentGET;
        ...
        if (randomNumber < 0.25) {
          ...
          return response.status(505).send({ error: "Server is busy" });
        } else if (body.uid && request.method === "GET") {
          ...
          if (profile)
            return response
              .status(200)
              .send({ status: "success", profile: profile });
        }
