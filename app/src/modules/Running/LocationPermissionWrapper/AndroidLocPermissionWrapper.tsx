...
const AndroidLocPermissionWrapper: React.FC<Props> = ({ children }) => {
  ...
  return (
    <View className="bg-black flex-1">
      {status?.granted && status.android?.accuracy === "fine" ? (
        children
      ) : status?.granted && status.android?.accuracy !== "fine" ? (
        <PermissionTaker
          imgUrl={trackingStepsPermissionImg}
          text="Please give access to fine accuracy to track a run"
          imgStyleTW="w-3/5 aspect-square"
          txtLayoutStyleTW="w-[85%]"
        >
          <View className="w-full mx-auto pb-10">
            <StartButton
              title="Give fine accuracy"
              bgColor="bg-[#fff]"
...
export default AndroidLocPermissionWrapper;
