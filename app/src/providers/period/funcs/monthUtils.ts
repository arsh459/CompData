...
import { CalendarMonth } from "../periodStore";
import { getObjMap } from "./dataUtils";
...
export const getPreviousMonthsFromString = (
  unix: number,
  count: number
): CalendarMonth[] => {
  const targetDate = new Date(unix);
...
export const getPeriodDateObjsInRange = async (
  uid: string,
  start: number,
  end: number,
  inDBSt: number,
  ...
  const objs = await getObjMap(uid, start, end);
  return {
    objs: objs,
    start,
    end,
...
export const parseToMarkedDates = (
  periodDateObjs: PeriodDateObj[],
  currentlySelectedDate: string
) => {
  const datesToUse: MarkedDates = {};
  for (const pObj of periodDateObjs) {
    if (currentlySelectedDate === pObj.date && pObj.type === "PERIOD") {
      datesToUse[pObj.date] = { color: "PERIOD", selected: true };
    } else if (pObj.type === "PERIOD") {
      datesToUse[pObj.date] = { color: "PERIOD" };
    }
  }
