...
const GiftPage: React.FC<Props> = () => {
  ...
  return (
    <DefaultLayout
      title={seoData.giftPage.title}
      description={seoData.giftPage.description}
      noIndex={false}
      link={seoData.giftPage.link}
      canonical={seoData.giftPage.link}
      img={seoData.giftPage.img}
    >
      {(!deviceType && query.device) || !router.isReady ? (
        ...
        <div className="fixed inset-0 z-0 bg-[#100F1A] flex flex-col text-white overflow-y-scroll scrollbar-hide">
          <GiftTemplate user={user} deviceType={deviceType} />
        </div>
      ) : (
        <div className="fixed inset-0 z-0 bg-[#100F1A] flex justify-center items-center">
          <Loading fill="#ff735c" width={48} height={48} />
