...
import {
  // parsePayQuery,
  parsePaySubscriptionQuery,
} from "@utils/payments/parsePayQuery";
import { withSentry } from "@sentry/nextjs";

const paySubscription = async (req: NextApiRequest, res: NextApiResponse) => {
  try {
    if (req.method === "POST") {
      const { appSubId, discount } = parsePaySubscriptionQuery(req.query);
...
export default withSentry(paySubscription);
