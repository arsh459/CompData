...
import BadgeSectionsTemplate from "@templates/AdminGames/BadgeSectionsTemplate";
...
const BadgeSectionPage: React.FC<Props> = ({ gameId, prizeId }) => {
  const { user, authStatus } = useAuth();

  return (
    <DefaultLayout
      title="Admin"
      link={`https://${homeDomain}/admin/games/${gameId}/${prizeId}/sections`}
      canonical={`https://${homeDomain}/admin/games/${gameId}/${prizeId}/sections`}
      img={boatsSEO.img}
      noIndex={true}
    ...
    >
      {authStatus === "PENDING" ? (
        ...
        <div>
          <p>List of all badgeSections</p>
          <BadgeSectionsTemplate prizeId={prizeId} gameId={gameId} />
        </div>
      ) : (
        //
        <div className="flex justify-center items-center text-4xl font-bold h-screen">
