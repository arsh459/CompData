...
import FAQForm from "@modules/LandingPageForm";
...
const LandingPageDetails: React.FC<Props> = ({ gameId, prizeId }) => {
  const { user, authStatus } = useAuth();

  return (
    <DefaultLayout
      title="Admin: Landing page form"
      link={`https://${homeDomain}/admin/games/${gameId}/${prizeId}/faq`}
      canonical={`https://${homeDomain}/admin/games/${gameId}/${prizeId}/faq`}
      img={boatsSEO.img}
      noIndex={true}
    ...
    >
      {authStatus === "PENDING" ? (
        ...
        <div>
          <FAQForm gameId={gameId} badgeId={prizeId} />
        </div>
      ) : (
        //
        <div className="flex justify-center items-center text-4xl font-bold h-screen">
