...
import PastAppointments from "@modules/Patients/Components/PastAppointments";

interface Props {
  remoteUser: UserInterface | null;
}

const PastAppointmentsPage: React.FC<Props> = ({ remoteUser }) => {
  ...
  return (
    <DefaultLayout
      ...
      link={`https://${homeDomain}/admin/patients/${remoteUser?.uid}/appointments`}
      canonical={`https://${homeDomain}/admin/patients/${remoteUser?.uid}/appointments`}
      img={boatsSEO.img}
      noIndex={true}
    ...
    >
      {authStatus === "PENDING" ? (
        ...
        remoteUser ? (
        <div>
          <PastAppointments remoteUser={remoteUser} />
        </div>
      ) : (
        //
        <div className="flex justify-center items-center text-4xl font-bold h-screen">
