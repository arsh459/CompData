...
import EnergyTracker from "@modules/ProgressModule/EnergyTracker";
...
const ProgressPage: React.FC<Props> = ({ remoteUser }) => {
  ...
  return (
    <DefaultLayout
      ...
      link={`https://${homeDomain}/admin/patients/${remoteUser?.uid}/progress/energy`}
      canonical={`https://${homeDomain}/admin/patients/${remoteUser?.uid}/progress/energy`}
      img={boatsSEO.img}
      noIndex={true}
    ...
    >
      {authStatus === "PENDING" ? (
        ...
        remoteUser ? (
        <EnergyTracker remoteUser={remoteUser} />
      ) : (
        <div className="flex justify-center items-center text-4xl font-bold h-screen">
