...
import WeightTracker from "@modules/ProgressModule/WeightTracker";
...
const ProgressPage: React.FC<Props> = ({ remoteUser }) => {
  ...
  return (
    <DefaultLayout
      ...
      link={`https://${homeDomain}/admin/patients/${remoteUser?.uid}/progress/weight`}
      canonical={`https://${homeDomain}/admin/patients/${remoteUser?.uid}/progress/weight`}
      img={boatsSEO.img}
      noIndex={true}
    ...
    >
      {authStatus === "PENDING" ? (
        ...
        remoteUser ? (
        <WeightTracker remoteUser={remoteUser} />
      ) : (
        <div className="flex justify-center items-center text-4xl font-bold h-screen">
