...
const ProgressPage: React.FC<Props> = ({ remoteUser }) => {
  ...
  return (
    <DefaultLayout
      ...
      link={`https://${homeDomain}/admin/patients/${remoteUser?.uid}/progress`}
      canonical={`https://${homeDomain}/admin/patients/${remoteUser?.uid}/progress`}
      img={boatsSEO.img}
      noIndex={true}
    ...
    >
      {authStatus === "PENDING" ? (
        ...
        remoteUser ? (
        <ProgressModule remoteUser={remoteUser} />
      ) : (
        <div className="flex justify-center items-center text-4xl font-bold h-screen">
