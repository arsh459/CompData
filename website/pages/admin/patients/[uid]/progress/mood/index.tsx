...
import MoodTracker from "@modules/ProgressModule/MoodTracker";
...
const ProgressPage: React.FC<Props> = ({ remoteUser }) => {
  ...
  return (
    <DefaultLayout
      title={`${
        remoteUser?.name ? `${remoteUser?.name} Mood Progress` : "Mood Progress"
      }`}
      link={`https://${homeDomain}/admin/patients/${remoteUser?.uid}/progress/mood`}
      canonical={`https://${homeDomain}/admin/patients/${remoteUser?.uid}/progress/mood`}
      img={boatsSEO.img}
      noIndex={true}
      description="Progress for the user Mood. Information is confidential and private."
    >
      {authStatus === "PENDING" ? (
        ...
        remoteUser ? (
        <MoodTracker remoteUser={remoteUser} />
      ) : (
        <div className="flex justify-center items-center text-4xl font-bold h-screen">
