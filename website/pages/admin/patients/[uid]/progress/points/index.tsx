...
import PointsMain from "@modules/ProgressModule/PointsMain";

interface Props {
  remoteUser: UserInterface | null;
}

const PointsPage: React.FC<Props> = ({ remoteUser }) => {
  ...
  return (
    <DefaultLayout
      ...
      link={`https://${homeDomain}/admin/patients/${remoteUser?.uid}/progress/points`}
      canonical={`https://${homeDomain}/admin/patients/${remoteUser?.uid}/progress/points`}
      img={boatsSEO.img}
      noIndex={true}
    ...
    >
      {authStatus === "PENDING" ? (
        ...
        remoteUser ? (
        <PointsMain remoteUser={remoteUser} />
      ) : (
        <div className="flex justify-center items-center text-4xl font-bold h-screen">
