...
import FlagsComponent from "@modules/Patients/Components/Flags";

interface Props {
  remoteUser: UserInterface | null;
}

const FlagsPage: React.FC<Props> = ({ remoteUser }) => {
  ...
  return (
    <DefaultLayout
      title={`${
        remoteUser?.name ? `${remoteUser?.name} Flags` : "Flags Appointments"
      }`}
      link={`https://${homeDomain}/admin/patients/${remoteUser?.uid}/flags`}
      canonical={`https://${homeDomain}/admin/patients/${remoteUser?.uid}/flags`}
      img={boatsSEO.img}
      noIndex={true}
    ...
    >
      {authStatus === "PENDING" ? (
        ...
        remoteUser ? (
        <div>
          <FlagsComponent remoteUser={remoteUser} />
        </div>
      ) : (
        //
        <div className="flex justify-center items-center text-4xl font-bold h-screen">
