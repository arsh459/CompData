...
const AddRoundsBenefit: React.FC<Props> = ({ uid, formData, setFormData }) => {
  ...
  const onDeletePrize = (prize: BenefitInterface) => {
    if (formData?.benefits) {
      const remoteSBPrizes: BenefitInterface[] = formData?.benefits?.filter(
        (each: BenefitInterface) => each !== prize
      );
      setFormData({ ...formData, benefits: remoteSBPrizes });
    }
  };
  ...
  const handleSavePrize = () => {
    if (prizeMedia.length > 0 && prizeMedia[0] && prizeName) {
      ...
      const remoteSBPrizes: BenefitInterface[] = formData?.benefits
        ? [...formData.benefits, remoteBenefit]
        : [remoteBenefit];
      setFormData({ ...formData, benefits: remoteSBPrizes });
      setPrizeMedia([]);
  ...
  return (
    <div className="border p-4">
      <h2 className="font-bold px-2">Add Benefits :</h2>
      ...
      {addPrize ? (
        <div>
          <div className="flex items-center p-4">
            <label className="pr-2">Benfit Text :</label>
            <input
              type="string"
      ...
      {formData?.benefits && formData?.benefits.length > 0 ? (
        <div className="grid grid-flow-col auto-cols-fr">
          {formData.benefits.map((each, index) => (
            <div
              key={each.text}
...
export default AddRoundsBenefit;
