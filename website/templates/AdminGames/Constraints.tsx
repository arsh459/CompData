...
const Constraints: React.FC<Props> = ({ uid, badge, setBadge }) => {
  ...
  const handleSaveTextAndImg = () => {
    if (textAndImg.icon && textAndImg.text) {
      const remoteTextAndImg: TextAndImgType[] = badge.constraints
        ? [...badge.constraints, textAndImg]
        : [textAndImg];
      setBadge({ ...badge, constraints: remoteTextAndImg });
  ...
  const onDeleteTextAndImg = (textAndImg: TextAndImgType) => {
    const remoteTextAndImg = badge.constraints?.filter(
  ...
  return (
    <div className="border p-4 m-4">
      <h2 className="font-bold px-2">Constraints :</h2>
      ...
      {badge.constraints && badge.constraints.length > 0 ? (
        <div className="grid grid-flow-col auto-cols-fr">
          {badge.constraints.map((each, index) => (
            <div
              key={`${each.text}-${index}`}
