...
const WeTeaches: React.FC<Props> = ({ uid, badge, setBadge }) => {
  ...
  const handleSaveTextAndImg = () => {
    if (textAndImg.icon && textAndImg.text) {
      const remoteTextAndImg: TextAndImgType[] = badge.weTeach
        ? [...badge.weTeach, textAndImg]
        : [textAndImg];
      setBadge({ ...badge, weTeach: remoteTextAndImg });
  ...
  const onDeleteTextAndImg = (textAndImg: TextAndImgType) => {
    const remoteTextAndImg = badge.weTeach?.filter(
  ...
  return (
    <div className="border p-4 m-4">
      <h2 className="font-bold px-2">We Teachs (Image Frame 55*55) :</h2>
      ...
      {badge.weTeach && badge.weTeach.length > 0 ? (
        <div className="grid grid-flow-col auto-cols-fr">
          {badge.weTeach.map((each, index) => (
            <div
              key={`${each.text}-${index}`}
